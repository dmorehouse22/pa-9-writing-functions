...
nums <- 3:12 
round(nums / min(nums, na.rm = TRUE), 0)
```
...
divide_and_round <- function(x){
round(x / min(x, na.rm = TRUE), 0)
}
```
...
no_nines_or_twelves <- function(x) {
ifelse(x %% 9 == 0 | x %% 12 == 0, FALSE, TRUE)
}
...
every_other <- function(x, start = 1){
